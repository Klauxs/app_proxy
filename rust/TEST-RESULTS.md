**2026-09-24：本地代理端口冲突自动恢复**

后续修复添加/复制实例的保存衔接：网络验证自动换端口并递增配置版本后，菜单原先仍传入确认前的旧版本，导致代理可用但实例未保存。现在仅对一次提交中纯本地端口/代理修订号的变化更新保存版本；额外提交或其他字段变化仍拒绝。菜单 6 项测试通过，其中新增真实端口占用 → 自动分配 → 旧版本拒绝 → 新版本创建/复制 → 重开验证持久实例，以及并发修改拒绝用例。实例 CLI 合约另有 12 项通过、4 项默认忽略；fmt 和全目标 Clippy 通过。日志：`.tools/port-recovery-instance-menu.log`、`.tools/port-recovery-instance-cli.log`、`.tools/port-recovery-instance-check.log`。

启动及 Down 恢复会保留可绑定的入口，只对绑定失败的入口申请 Windows 自动端口，持久保存后继续健康检查；不因已有应用绑定而阻止恢复。共享代理扩展/配置切换把端口调整与候选配置一起记录，失败回滚遇到旧端口冲突时也会自动换端口。新应用使用已确认 generation 的入口，前台提示旧应用重启；不接管或结束外部监听程序。

- `verify.ps1`：520 项通过、65 项默认忽略；最终 fmt、Clippy（警告视为错误）通过。
- 显式运行真实 sing-box 的 4 项切换集成测试全部通过：移除、扩展、手动配置更新、订阅节点切换/刷新，包含失败回滚。
- 真实 sing-box + 本地 TLS 健康检查测试通过：初次启动时两个端口被占用仍成功；退出后恢复时保留未冲突入口、自动调整冲突入口；外部监听仍可连接；重开配置后复用新的内核身份和端口。
- 扩展测试新增“确认后新端口被占用”和“候选失败后旧端口被占用”的恢复场景；后者保留实例绑定并在新入口验证旧路由连通。存储测试验证重绑定候选跨重开提交，以及拒绝夹带其他配置修改。
- 本轮使用隔离测试目录及本机已安装的 sing-box 二进制，没有替换本机安装版本，也没有在用户报错的另一台设备上验证。

验证日志：`.tools/port-recovery-verify.log`、`.tools/port-recovery-final-check.log`、`.tools/port-recovery-real-core.log`、`.tools/port-recovery-real-health.log`。

**2026-09-23：Claude 旧版 MSIX 容器冲突恢复**

新增仅在同次 Windows 208/215 事件确认共享冲突后触发的旧包残留清理；相同用户/会话、严格旧版完整包身份、精确进程句柄、一次性票据锁和一次重试共同限制范围。普通启动不扫描，不改变 Guard 主进程处理路径。诊断持久化到请求目录 `recovery.json`。

包相关回归 28 项和启动引擎回归 43 项通过；Windows/App 全目标 Clippy 通过。显式现场恢复测试仅结束旧 Claude 身份的 Git PID 8544，旧容器消失，其他 Git 保留，包 helper 激活恢复。随后现有安装版本成功启动真实 Claude，确认 PID 14588 带代理参数。新恢复逻辑尚未部署到已安装版本；未验证聊天流量。详见 [容器恢复设计及现场证据](docs/22-msix-container-recovery.md)。
...
默认关闭的异步计时已加入。37 项 Guard 回归、6 项进程查询测试通过，Release/Clippy/fmt/diff check 通过；真实管理员安装 + active ETW 六次自动纠正均通过。Codex/Claude 创建到退出平均 730.1/1147.2 ms，WMI 分别 471.6/699.4 ms；Claude 三次均出现一次完整重采样。日志无丢弃，测试进程服务已清理，原 Codex 保留。详细口径与原始证据见 [分段实测](docs/decisions/2026-09-20-guard-timing.md)。未将先前 446 项全量回归标为本轮重新执行。
