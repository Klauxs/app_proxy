...
`status` 同时返回启用意图、实际状态、监听/IFEO 组件状态和只读进程观察。缺少监听组件时显示 `needs_authorization`；只读核验受保护部署及任务成功后仍为 `unverified/blocked`，并说明运行连接未确认，不以注册代替实际保护。只管理分身时 IFEO 为 `not_applicable`。进程扫描与组件核验共享 3 秒截止，超时返回配置状态和诊断；实际未结束工作仍持有名额，避免不断累积线程。扫描繁忙时以 `GUARD_SCAN_BUSY` 表示本次没有新的进程观察。
