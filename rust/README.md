...
代理健康检查的连接超时为 8 秒，单次请求总超时为 15 秒。超时或明确的临时网络连接错误最多重试 5 次（连同首次共 6 次），首次重试前等待 1 秒，后续等待 2 秒；整轮检测最多 30 秒，未用完次数也会在期限到达时结束。重试只通过同一代理入口重新发送检测请求，不重启 sing-box、不重复创建实例、不回退直连。配置、证书等未经确认可重试的错误及不符合要求的响应立即失败；回执保留连接超时、响应超时、总期限、TLS/传输、HTTP 状态等具体错误类别。

共享代理启动、退出后恢复以及已确认的配置切换会优先复用保存的本地端口。绑定失败时由 Windows 重新分配可用端口，只调整冲突入口，保存后继续启动和健康检查；已有应用依赖旧端口不会阻止恢复，使用旧端口的应用需要重启。外部监听进程不会被接管或结束。配置切换中的端口调整与候选配置一起持久记录；回滚时旧端口不可用也会自动恢复到新端口。前台会提示端口变化；本次新启动的应用直接使用新端口。添加或复制实例时，若验证期间仅发生一次自动端口恢复，会沿用用户确认并按新版本保存实例；其他并发配置修改仍需重新确认。检查与 sing-box 接管端口之间仍存在短暂竞争窗口，进程归属或启动结果未确认时仍需按原请求编号核对，不盲目重复创建。
