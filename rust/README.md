...
共享代理启动、退出后恢复以及已确认的配置切换会优先复用保存的本地端口。绑定失败时由 Windows 重新分配可用端口，只调整冲突入口，保存后继续启动和健康检查；已有应用依赖旧端口不会阻止恢复，使用旧端口的应用需要重启。外部监听进程不会被接管或结束。配置切换中的端口调整与候选配置一起持久记录；回滚时旧端口不可用也会自动恢复到新端口。前台会提示端口变化；本次新启动的应用直接使用新端口。检查与 sing-box 接管端口之间仍存在短暂竞争窗口，进程归属或启动结果未确认时仍需按原请求编号核对，不盲目重复创建。
