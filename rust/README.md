...
状态：基础平台、配置/存储、认证管道、协调进程、实例配置、共享 sing-box 管理与普通 EXE 启动 CLI 已实现。启动流程已接入缺失内核安装、共享代理扩容确认、查询与取消。MSIX 生产启动、Guard/IFEO、中文菜单及真实应用完整验收仍待实现，不能作为正式启动器使用。详见 [验证记录](D:/app_proxy/rust/TEST-RESULTS.md) 和 [功能进度](D:/app_proxy/rust/IMPLEMENTATION.md)。
...
相同请求编号查询历史结果，不重复安装或创建应用；结果不明时保留编号，使用 `launch inspect` 查询。Ctrl+C 请求取消尚未创建的应用，并阻止当前流程的后续动作；已创建的应用会保留。已接受的内核切换仍需按其原请求编号查询结果。当前只验证了隔离 EXE 夹具，MSIX 启动和 Guard/IFEO 尚未实现，启动成功不代表保护生效。
