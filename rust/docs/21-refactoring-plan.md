...
计划用 Codex 做独立评审。本机 Codex CLI 为 0.141.0，账号可用的模型要求更新版 CLI，未能运行；改由两个不带上下文的评审代理按同一份提示只读审查 `5adc708..HEAD`，一个负责协调进程与存储，一个负责 unsafe 封装、检查点、错误码与退出码。两份评审都没有发现当前会触发的问题，确认了请求路由改写前后逐条路径等价、39 个结果不确定的错误码全部登记、退出码表逐格一致、16 处 JSON 输出改写无误。它们指出的问题及处理：

| 问题 | 处理 |
|---|---|
| 迁移前保存原始字节用的是“创建后写入”，中断后会留下残缺文件并在下次被当作已保存 | 已修：改为与 manifest 相同的原子替换 |
| 旧版本文件走通用 JSON 值，重复的 `schema_version` 会取最后一个；每次加载解析两遍 | 已修：版本号由严格的头部结构读取，当前版本只解析一遍 |
| 客户端用线上可表示的错误码判断“请求未发出” | 已修：改为本地的投递结果区分，不再依赖错误码 |
| 安装闸门改成类型化读取后变严，`published` 为其他形状时由“保持关闭”变为报错，会使协调进程的服务循环直接退出 | 已修：恢复按通用值读取，并补测试 |
| `intern` 的容量上限会波及调用方要匹配的具名错误码 | 已修：具名错误码先于上限返回 |
| 未决的配置请求和内核更新计划内嵌 manifest，首次升 schema 版本时会使 store 打不开 | 未触发；规则写入 [02-model-and-storage.md](02-model-and-storage.md) 第 8 节 |
| 启动日志缺失判断由 `try_exists` 改为匹配 NotFound 后，路径上是悬空重解析点时由“视为空”变为 `STORE_REPARSE_POINT` | 保留：更严格，符合不变量 6 |
| JSON 编码失败的错误码统一为 `OUTPUT_ENCODING_FAILED`，原有 5 处为 `OUTPUT_SERIALIZE_FAILED` | 保留：这些类型的序列化不会失败 |
| 三个测试在宿主有 MSIX 包身份时跳过并计为通过；子进程测试路径若过期，子进程会运行零个测试并以 0 退出 | 保留并注明：验收应在普通终端运行。模块分组那次的过期路径实际表现为 19 个测试失败，说明父测试会察觉 |
