...
| 1.5 | 短期记录的版本判断收拢到一个辅助函数，统一“版本不符则保留并报告”的行为；`core_update.rs:555` 对 v1 计划的特例保留 |
| 1.6 | 安装器日志的类型和文件名常量移入 `app_proxy_windows::setup`，安装器和启动闸门共用同一份类型，去掉 `setup.rs` 里对 `"published"` 键的无类型读取 |
| 1.7 | `package.ps1` 的版本号改为读取工作区版本 |

行为变化：1.2 新增一个错误码；1.5 可能改变个别版本不符时的错误码，逐个列在提交说明里。
...
| 2.5 | 给 `Operation` 增加一个方法返回执行类别（同步、需准入的后台任务、有界查询）。`handle()` 按执行类别拆成三个函数；删除 `execute()` 里对已拦截变体返回 `*_REQUIRES_ADMISSION` 的分支，改为类型上不可达 |
| 2.6 | 待评估：客户端每次操作前的状态往返能否并入握手。需要先确认它是否承担了“确保协调进程已启动”之外的职责 |
...
|---|---|---|
| 0 基线与护栏 | 未开始 | |
| 1 格式升级策略 | 未开始 | 发布前必须完成 |
| 2 拆分协调进程 | 未开始 | 2.6 待评估 |
| 3 统一请求日志 | 未开始 | |
| 4 收口 Store | 未开始 | |
