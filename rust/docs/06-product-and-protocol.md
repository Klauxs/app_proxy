...
当前 Rust coordinator 协议为 2.11。启动、启动请求查询及显式取消要求服务端 minor 至少为 7，带 expected_revision 的启动要求至少为 8，Guard 状态要求至少为 10。订阅协议扩展后的 Catalog 要求两端 minor 至少为 11；旧客户端不会收到无法解析的新协议枚举，旧服务端也不会被当作支持订阅目录。新启动请求只包含已登记实例 ID、来源及可选版本条件，回执不含参数、环境值或凭据；接纳 ACK 不表示创建成功，客户端查询持久 attempt 阶段。中文完整菜单及订阅导入/刷新入口继续按本章设计接入。
