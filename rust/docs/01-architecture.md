...
2026-09-20 用户取消 IFEO。Guard 仅使用 ETW 启动后检查与已授权监听失效时的周期扫描；原版与分身都按实际监听/扫描证据判断状态。未管理原版保持不干预。通过本工具启动时先准备代理，从外部入口启动可能在检查前已执行或联网，Guard 不承诺启动前拦截。决定见 [第九章](09-ifeo-launch-interception.md)。
