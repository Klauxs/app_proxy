...
收到已认证批次才报告 ETW 连接，结束批次立即降级，5 秒无新批次视为心跳过期。读取独占任务不会为定时器取消一半的帧。初连、事件提示和丢失只合并为一个待扫描通知；连接正常每 30 秒补扫通知，已验证组件但断线时每 2 秒通知，用户应用保留。此批尚无自动扫描消费者，状态明确显示扫描待接入，不能报告 Guard active。
