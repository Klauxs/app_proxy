...
通过普通协调进程环境 `APP_PROXY_TIMING_FILE` 开启有界异步计时，默认关闭，高权限进程不通过此配置写日志。覆盖启动事件送达、首次扫描、候选查询、WMI 连接/查询/读取、资源准备、关闭前复核、停止授权及原生终止。真实六次纠正通过：WMI 占总耗时约 61%～65%，Claude 额外一次重采样使其三次均超过 1 秒；尚未改用其他参数读取方式。测试及清理详情见 [分段实测](docs/08-guard-timing-2026-09-20.md)。
