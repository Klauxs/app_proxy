...
| 实例启动 | 普通 EXE LaunchEngine 服务、持久状态、core 许可及跨 store 预留已实现，独立 review 通过；产品入口待接入 | 去重/取消、实际夹具创建、外部占用拒绝、配置复核、双 journal 恢复及精确退出通过；RPC/CLI、真实 Codex/Claude、多实例完整验收、MSIX 与无可信回执的未知结果核对待实现 |
