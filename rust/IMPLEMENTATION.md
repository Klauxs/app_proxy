...
最近完成：快捷方式持久归属与显式恢复平台通过独立 review。发布前保存临时文件身份，同句柄改名且不覆盖；中断后核对文件与最新 manifest，同一请求不会重复创建，用户修改和未知结果保留。未清理入口阻止删除实例，新增前预留完成/删除所需记录空间。26 项专项经独立复跑通过。coordinator RPC 和菜单创建入口继续接入；高级设置、维护和完整产品验收仍待完成。范围见 `TEST-RESULTS.md`。

| 功能 | 当前状态 | 完成证据 / 下一项验证 |
|---|---|---|
| 中文日常菜单 | 实例/代理日常流程及独立进程状态已接入，独立 review 通过；完整菜单待续 | 无参数/menu 入口，默认原版、明确网络选择、流程内安装/保护/启动、共用回执及版本核验；3 项真实 PTY 通过，Ctrl+C 完整断言待实机；高级设置、快捷方式和维护入口待续 |
...
| 快捷方式 / 图标平台 | 原生平台、host 启动及持久事务已实现，独立 review 通过；RPC/菜单待续 | 完整图标提取实测、隐藏启动及 3 项原生 UI/取消契约；新增所有权、创建/删除断点恢复、实例保护、容量及请求冲突回归；菜单创建与真实桌面点击待续 |
