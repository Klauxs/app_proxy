...
最近完成：host 隐藏启动、依赖前台选择和故障通知通过独立 review。共用既有 LaunchEngine/CLI 流程，正常成功无输出，确定依赖缺失/需确认时才显示控制台；原生控制台句柄/首提示前 Ctrl+C、缺内核选择返回、消息框请求编号均实测并独立复跑。快捷方式事务与菜单创建入口继续接入；高级设置、维护和完整产品验收仍待完成，之前菜单 Ctrl+C 的 PTY 完整断言不因这次独立控制台测试而计为通过。范围见 `TEST-RESULTS.md`。
...
| 快捷方式 / 图标平台 | 原生平台及 host 启动已实现，独立 review 通过；创建接入待续 | Shell Link 与图标平台 11 项默认回归及 Codex/Claude 图标实测；host launch --notify、共用前台操作、原编号故障提示和 3 项原生 UI/取消契约通过；事务恢复、菜单创建与真实桌面点击待续 |
