...
最近完成：中文日常菜单及共用 CLI 流程通过独立 review。新增无终端零写入、默认选择/EOF、超长秘密行回归；真实 PTY 验证原版保存后返回/改名/拒绝不支持的分身、缺 sing-box 时返回且保留代理、并发修改后拒绝旧摘要。独立复跑 16 项相关默认测试及两项真实 sing-box CLI 回归通过；PTY 的 Ctrl+C 会中断测试宿主，完整取消断言未验证。无参数菜单只展示已接通操作，完整运行状态、高级设置、快捷方式、维护和产品验收仍待续。实现与验收范围见 `TEST-RESULTS.md`。

| 功能 | 当前状态 | 完成证据 / 下一项验证 |
|---|---|---|
| 中文日常菜单 | 实例/代理日常流程已接入，独立 review 通过；完整菜单待续 | 无参数/menu 入口，默认原版、明确网络选择、流程内安装/保护/启动、共用回执及版本核验；3 项真实 PTY 通过，Ctrl+C 完整断言待实机；关闭 Guard 的运行状态、高级设置、快捷方式和维护入口待续 |
