...
ETW 回调入队立即唤醒发送，固定 25 ms 刷新；事件任务复用已固定的程序/目录句柄，将两个进度状态写入合并到持久停止意图。取消、配置、精确身份、资源预留和恢复规则保持。默认回归合计 454 项通过、60 ignored，Release/Clippy/fmt/diff check 通过。真实 Codex 三次 49.799 / 95.200 / 104.073 ms（平均 83.0），Claude 84.694 / 114.441 / 117.405 ms（平均 105.5），六次均带代理重启，关闭前 WMI 全部为 0。修复测试清理工具的 PID 复用问题后确认 48 个测试应用进程退出；核心、辅助及协调服务停止，保护禁用、登录入口移除，原 Codex 保留。详见 [分段、开销与验证](docs/decisions/2026-09-20-guard-wakeup.md)。

**2026-09-20 最新实测：原生逐事件路径六次均在 300 毫秒内关闭**

管理员授权完成，生产 Release 对已安装 Codex/Claude 空白隔离实例各测试三次，均精确关闭并带代理重启。Codex 296.692 / 238.965 / 269.275 ms，Claude 212.738 / 249.387 / 273.367 ms；目标关闭前 WMI 全部 0 次，原生命令行读取 0.276～0.413 ms。上一轮默认回归合计 451 项通过、60 ignored，Release/Clippy/fmt/diff check 通过；本轮仅继续真实实测及文档收尾。50 个测试应用进程及核心/辅助/协调进程已退出，保护禁用、测试登录入口移除，原 Codex 保留，事件任务 Ready。六个样本不构成任意负载的延迟上限。详见 [实现、分段与验证](docs/decisions/2026-09-20-guard-native-events.md)。

**2026-09-20 最新诊断：可选分段计时已实现**

通过普通协调进程环境 `APP_PROXY_TIMING_FILE` 开启有界异步计时，默认关闭，高权限进程不通过此配置写日志。覆盖启动事件送达、首次扫描、候选查询、WMI 连接/查询/读取、资源准备、关闭前复核、停止授权及原生终止。真实六次纠正通过：WMI 占总耗时约 61%～65%，Claude 额外一次重采样使其三次均超过 1 秒；尚未改用其他参数读取方式。测试及清理详情见 [分段实测](docs/decisions/2026-09-20-guard-timing.md)。

**2026-09-20 前次验证：退出竞态修复版完成六次真实自动纠正**

管理员授权成功后，生产 Release 在 active ETW 下对已安装 Codex、Claude 各验证 3 次，均成功关闭未代理目标并带代理重启。本轮未复现前次错误；Codex 关闭耗时 0.880 / 0.773 / 0.881 秒，Claude 1.059 / 1.255 / 0.982 秒，仍不能声明稳定低于 1 秒。测试应用和服务已清理、保护已禁用，原 Codex 保持运行。未修改生产代码或重复默认回归，详见 [验收记录](ACCEPTANCE-2026-09-20.md)。

**2026-09-20 前次修复：处理 Guard 候选退出竞态**

已实现关闭前对已知进程消失错误进行有界完整重新采样，目标自行退出时停止本次纠正；新 Absent 扫描可解除无停止意图的已知临时失败。身份、权限、配置/取消核验和停止后失败保留不变。默认全量 446 通过、0 失败、59 ignored，Release/Clippy/fmt/diff check 通过；Windows 管理员授权被取消，真实应用补测未启动，尚无修复后速度结论，测试保护及协调进程已关闭。详见 [验收记录](ACCEPTANCE-2026-09-20.md)。

**2026-09-20 前次迭代：目标 1 秒内**

已实现每次原生核对包登记/路径/清单摘要后的有界解析缓存、自有 ETW 100ms 主动刷新和 Ready 50ms 合并等待。完整身份、配置/取消、预留、授权及未知退避保持。442 项默认测试通过、0 失败、59 ignored，1 项真实包只读一致性补验通过，Release/clippy/fmt/diff check 通过。真实 UAC + ETW 补验已执行：Codex 0.784 / 1.135 / 0.749 秒，Claude 首次 0.744 秒；Claude 第二次关闭前核验因进程退出而失败，第三次受历史失败状态阻塞未启动。尚未达到稳定 1 秒内，且存在实际纠正失败，需继续定位；不能将默认测试通过作为整轮验收成功。详见 [验收记录](ACCEPTANCE-2026-09-20.md)。

**2026-09-20 前次优化：优先缩短未代理进程存活时间**

Guard 使用立即精确终止策略，不再等待正常退出；关闭前合并两次独立主进程查询至同一批候选核验。保留身份、配置/取消、预留、停止授权、辅助进程退出及重启检查。438 项默认测试通过、0 失败、58 ignored，Release/clippy/fmt/diff check 通过。用户再次要求测速后完成 UAC 授权，优化版原生创建到退出实测 Codex 2.114 秒、Claude 3.325 秒；Claude 重启通过，Codex 关闭后因代理健康失败保持关闭，未直连回退。单次样本不代表稳定上限或 p95。测试保护和服务已关闭，详见 [验收记录](ACCEPTANCE-2026-09-20.md)。

**2026-09-20 前次修复：真实应用 Guard 自动纠正通过**

Codex 的 Guard 候选归属查询改为同一轮批量 WMI 查询，保留句柄与前后身份核验；Claude 的辅助进程退出确认改为关闭主进程前保留确切句柄，避免退出中的映像查询 Win32 5 中断重启。普通启动不扫描外部占用的决定保持不变。437 项默认测试通过、0 失败、58 ignored，Release/clippy/fmt/diff check 通过；真实 Codex 与 Claude 的空白隔离实例均完成未代理启动→自动关闭→带代理重启。证据与验收边界见 [最新验收记录](ACCEPTANCE-2026-09-20.md)。以下保留修复前记录；账号业务等其余产品验收未据此完成，旧压缩包未更新。

**2026-09-20 修复前阻塞：真实应用 Guard 自动纠正**

真实 Codex/Claude 的补验均未通过：Codex 存在 Guard 扫描超时，实际未代理实例在 75 秒内未被纠正；Claude 的目标关闭成功，但重启准备以 `LAUNCH_PREPARATION_FAILED` 失败，具体底层原因尚未确定。此前受控 EXE 成功仍有效，但不能视作这两款应用的生产链路验收通过。下一步应定位并修复这两个实际阻塞，再复跑真实应用纠正。证据、范围和清理见 [验收记录顶部](ACCEPTANCE-2026-09-20.md)。本轮没有生产代码变更。

**2026-09-20 后续实测：管理员 ETW 与生产 Guard**

原生管理员 ETW 三项测试通过；当前 Release 的 UAC 安装→受保护事件任务→普通侧 `active_etw`→误启动自动纠正已在隔离受控 EXE 上通过。已验证重启后的真实 sing-box 转发、同映像未登记进程保留，以及上游故障时关闭目标并拒绝直连回退。证据和范围见 [验收记录顶部](ACCEPTANCE-2026-09-20.md)。真实 Codex/Claude 业务、注销登录触发和其他产品验收仍未据此完成；本轮没有生产代码变更。以下历史状态以这次实测补充为准。

**2026-09-20 后续决定：普通启动不判断外部进程是否运行**

已移除普通 CLI/菜单/快捷方式启动前的两处外部占用扫描；让应用处理自己的单实例和数据目录占用。本工具请求去重、可信会话复用和跨 store 自有资源预留仍保留，Guard 纠正仍核验身份与占用。Release 真实 Claude/Codex 各两个分身同时启动、详情和重复请求复用通过，先前 Codex B 启动超时场景已消除。完整产品其余验收范围见 [最新验收记录](ACCEPTANCE-2026-09-20.md)。以下保留此前记录。
...
活动集合移除已接入 CLI 和中文菜单的预览/确认/回滚流程，最后入口移除会停止内核后提交配置。新增 `core recover-start`，使用创建前持久化的 Job 归属证据核对启动中断；同一机制覆盖重配置/回滚中断。IPC 3.20。旧版无证据的 Starting 不自动接管，Down 集合先显式停止后删除。当前批次验证见 TEST-RESULTS.md。IFEO、跨目录升级、登录任务被删后的修复维持取消。

**实现与验收跟踪**

目标：按已确认设计完整实现 Rust 版，每项功能经独立 review、修复与验证后单独 commit。此文件记录进度，不缩小设计范围；不把实验、编译或局部测试作为整个产品完成的证据。

当前开发：2026-09-20 按用户决定取消 IFEO，移除注册/恢复、入口解析和调试脱离候选；原版 Guard 仅依据监听/扫描判断状态。IPC 3.19 删除 IFEO 状态及来源，旧 major 握手拒绝。此前快捷方式检查/丢失恢复及登录任务工作保留。全量串行 430 通过、0 失败、53 ignored，clippy/fmt/diff 通过，详见 TEST-RESULTS.md；真实登录/Shell 点击、完整 UAC 链、保留数据卸载及产品验收仍待完成。本批未提交。

| 功能 | 当前状态 | 完成证据 / 下一项验证 |
|---|---|---|
| 中文日常菜单 | 实例/代理日常流程、进程状态、快捷方式及高级设置已接入，独立 review 通过；完整菜单待续 | 无参数/menu 入口，默认原版、明确网络选择、流程内安装/保护/启动、共用回执及版本核验；快捷方式及高级设置真实 PTY 通过；维护及既有 Ctrl+C 完整断言待续 |
| 实例只读运行状态 / 详情 | 已实现，独立 review 通过 | RuntimeStatus IPC 2.15、instance inspect 和菜单接入；历史完整身份/网络与配置关系、外部归属、退出后重新观察、未知/繁忙/超时、版本及并发请求核验；不改 journal 或接管外部进程，不证明网络健康/目标流量 |
| 快捷方式 / 图标平台 | 原生平台、host、持久事务、RPC/CLI/菜单已实现，独立 review 通过；完整验收待续 | 完整图标、隐藏启动及 3 项原生 UI/取消契约；持久断点与所有权、IPC 2.16/忙时查询/断线、真实 CLI 和桌面/菜单创建删除通过；真实 Shell 点击、更新修复待续 |
| M0 普通进程/身份 | 普通创建已实现；调试创建已随 IFEO 取消移除 | 普通 child 参数/环境/身份测试，不等于完整 M0 通过 |
| MSIX 包内 helper | 生产执行链及恢复已接入，独立 review 通过；完整验收待续 | Claude/Codex 各两个直连分身同时运行、目录写入隔离及重复请求复用通过；Codex 原版共存通过；登录、实际代理及更新验收待续 |
| 配置模型 | 手动和订阅来源模型已实现，独立 review 通过 | 严格 JSON、引用/路径/Guard 校验；HTTP/SOCKS5 与六协议订阅元数据/秘密引用，读取和提交校验完整 typed 秘密；刷新编辑入口待续 |
...
| 共享 core 手动上游重配置 | 已实现，独立 review 通过 | 检查候选、影响预览/具体计划确认、原程序切换、失败回滚、持久化阶段/回执恢复；活动集合移除与新创建 Starting 核对已接入；旧版无证据仍保留未知，普通 EXE 启动许可已接入 |
...
| sing-box 管理/一键安装 | 初始生命周期、coordinator/CLI、一键安装/取消已实现，独立 review 通过；产品集成待续 | 自有共享进程；固定官方包校验、自动目录、交互确认/重试、并发安装、进度与取消；启动许可、重配置/回滚和新版内核创建核对已实现；持续故障通知及完整产品验收待完成 |
...
| IFEO | 已取消并移除（2026-09-20） | 不再属于开发/验收范围；只保留外部 Debugger 只读拒绝及固定空配置占位，见第九章 |
| Guard/ETW | 监听 host、授权安装、普通侧监听监督、自动扫描与纠正触发已接入 | 调度/受控进程/状态/协议测试及独立 review 通过；真实提权事件链路及登录触发仍待验收 |
...
| Guard 普通登录任务平台与持久事务 | 平台、RPC、Guard 启用和诊断已接入，独立 review 通过 | 当前 SID 登录/LUA/固定 host 和 store UUID；恢复、工作租约、真实任务中断及忙时历史重放通过；只读核验原 home/任务/受保护映像，普通 Guard 状态独立展示；真实登录触发、UAC 完整启用和维护流程待续 |
...
| Guard CLI / 授权状态边界 | 已实现，独立 review 通过；完整激活待续 | status/enable/disable、desired/实际/组件分离；原版与分身共用监听/扫描就绪标准，交互启用可授权安装监听组件；尚未就绪返回 requires_action/5；查询不会激活保护 |
...
| 实例高级设置 | 已实现，独立 review 通过 | IPC 2.17；CLI 私密文件及菜单隐藏输入、args/cwd/env 部分编辑、秘密分存/恢复、CAS/旧启动计划拒绝、保留运行实例及下次启动实测 |
| 集成维护 | 快捷方式核验与丢失恢复已接入，完整维护待续 | 持久修复请求、原位置恢复、历史重放、修改/替换冲突保留；跨目录升级、登录任务被删除后的修复已取消；保留应用/数据卸载待实现 |
...
- 快捷方式操作接入：独立 review 通过，提交主题 `feat(rust): connect desktop shortcuts to coordinator and menu`。新建锁外读取安装/图标，锁内重放优先于版本复核；查询不恢复副作用，删除/恢复使用原记录且不依赖应用仍安装；取消绑定创建 ID 防止同 revision 新旧请求混淆。独立 11 项（10 默认、1 原生桌面）通过，主线另有交互菜单真实 PTY 创建/删除验证。

- 快捷方式持久事务：独立审查发现未来/逆序 revision 与后续记录容量未预留两项问题，均修复并复审通过；独立复跑 26 项专项。归属在解除前不随请求 TTL 过期，只有显式重试恢复外部操作；记录身份前遗留的不可启动临时文件不按名字认领。提交主题 `feat(rust): persist shortcut ownership and explicit recovery`。coordinator 与菜单尚未接入。

- IFEO 可行性证据复核：独立审查确认现有 host 重建子进程/DebugDetach 无法恢复 Chromium broker 原有进程/线程句柄和沙箱创建契约。已在第九章记录官方说明、当前源码与本地实现的边界；这不是 Codex/Claude 本机 IFEO 失败实测。暂不开放真实规则，当时保留完整要求等待用户决定；2026-09-20 用户已取消 IFEO；先继续独立的订阅功能。提交主题 `docs(rust): record IFEO Chromium continuation limitation`。
