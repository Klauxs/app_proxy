...
最近完成：登录任务持久事务与显式恢复通过独立 review。意图先落盘、COM 在配置锁外执行、工作租约持续到记账完成；终态重放只读，取消/删除绑定原创建，跨请求命名空间互斥。11 项默认专项及真实 Windows 注册/删除中断恢复通过，全量 434 项通过。RPC/Guard 启用界面接入、入口维护、真实登录/Shell 点击和完整产品验收仍待完成。范围见 `TEST-RESULTS.md`。
...
| Guard 普通登录任务平台与持久事务 | 已实现，独立 review 通过；流程接入待续 | 当前 SID 登录/LUA/固定 host 和 store UUID；恢复前身份核验、复用/冲突保留/空闲解除；持久请求、工作租约和恢复/容量专项及真实任务中断恢复通过；RPC/Guard 启用接入、真实登录触发待续 |
