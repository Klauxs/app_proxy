...
最近完成：实例高级设置通过独立 review，复用配置事务接入 CLI/菜单。参数和目录替换、环境设置/移除/恢复继承、秘密分存与版本校验已实现；真实 CLI 和交互菜单保存通过，运行实例保留及下次启动读取新值通过。入口维护、真实 Shell 点击和完整产品验收仍待完成。范围见 `TEST-RESULTS.md`。
