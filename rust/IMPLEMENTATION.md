...
最近完成：登录任务 RPC 与 Guard 启用流程通过独立 review。IPC 2.18 接入创建/显式恢复/历史查询/只读就绪核验，监听组件就绪后自动登记普通登录入口；当前保护与下次登录就绪分开显示。修复忙时历史重放、独立诊断不阻断关闭、版本闭环及原 home 迁移/替换误报；独立复跑 10 项验证，全量 443 项通过。入口维护、真实登录/Shell 点击、完整 UAC 链和 IFEO 生产接入及产品验收仍待完成。范围见 `TEST-RESULTS.md`。
...
| 集成维护 | 待实现 | 入口修复及保留应用/数据卸载；日常菜单和快捷方式平台进度见上方 |
