...
最近完成：快捷方式 coordinator/CLI/菜单接入通过独立 review。固定桌面/host/图标，查询与显式恢复分离，删除绑定显示的创建请求；单工作槽、断线继续、锁外资源查询和最终版本校验。真实桌面 CLI 与交互菜单创建/删除均通过，独立复跑默认回归及原生桌面契约。高级设置、入口维护、真实 Shell 点击和完整产品验收仍待完成。范围见 `TEST-RESULTS.md`。

| 功能 | 当前状态 | 完成证据 / 下一项验证 |
|---|---|---|
| 中文日常菜单 | 实例/代理日常流程、进程状态及快捷方式已接入，独立 review 通过；完整菜单待续 | 无参数/menu 入口，默认原版、明确网络选择、流程内安装/保护/启动、共用回执及版本核验；快捷方式创建/删除真实 PTY 通过；高级设置、维护及既有 Ctrl+C 完整断言待续 |
...
| 高级设置 / 集成维护 | 待实现 | 实例高级设置、入口修复及保留应用/数据卸载；日常菜单和快捷方式平台进度见上方 |
