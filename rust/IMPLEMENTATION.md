...
最近完成：原生 Shell Link 与完整图标缓存平台通过独立 review。只发布新链接，核对文件身份/内容/目标后精确删除，用户改动保留；Codex/Claude 实际 EXE 全尺寸图标提取、Windows 解码及临时链接往返均经独立复跑。快捷方式事务、host 隐藏启动/通知及菜单入口下一步接入；高级设置、维护和完整产品验收继续实现，之前 Ctrl+C 的 PTY 完整断言仍待实机。范围见 `TEST-RESULTS.md`。
...
| 快捷方式 / 图标平台 | 原生平台已实现，独立 review 通过；产品接入待续 | Shell Link 编码/不覆盖发布/精确删除、输出目录链固定；完整图标组提取及 protected hash cache；11 项默认回归及 Codex/Claude 实机只读提取通过；事务恢复、host launch、菜单与真实桌面点击待续 |
