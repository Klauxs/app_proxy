...
最近完成：普通 coordinator 登录任务平台通过独立 review。固定普通权限/当前用户登录/发行版 host/预期 store，任务冲突保留；真实注册、精确回读、解除和遗留恢复通过。修复真实注册时 URI 规范化兼容问题，store 预期身份在任何配置恢复前校验。平台尚未接入 Guard 启用持久事务；入口维护、真实登录/Shell 点击和完整产品验收仍待完成。范围见 `TEST-RESULTS.md`。
...
| Guard 普通登录任务平台 | 已实现，独立 review 通过；流程接入待续 | 当前 SID 登录/LUA/固定 host 和 store UUID；恢复前身份核验、复用/冲突保留/空闲解除；真实普通任务注册回读删除已通过，持久意图及 Guard 启用接入、真实登录触发待续 |
