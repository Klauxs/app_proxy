...
最近完成：独立只读运行状态、菜单列表/详情与 `instance inspect` 通过独立 review。关闭 Guard 仍可查询进程，历史会话保留启动时网络，当前配置改变明确区分；外部进程只观察，未知/多主/超时不判作未运行。独立复跑 9 项相关回归通过，包括受保护回执只读、真实进程、协议版本及 CLI。高级设置、快捷方式、维护和完整产品验收继续实现；上一批 Ctrl+C 的 PTY 完整断言仍待实机。范围见 `TEST-RESULTS.md`。
