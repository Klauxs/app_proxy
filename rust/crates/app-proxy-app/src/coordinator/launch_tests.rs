...
#[tokio::test]
async fn core_protocol_requires_minor_twenty_before_either_admission() {
    ...
    let operations = || {
        [
            ...
            Operation::ControlCore {
                action: app_proxy_core::core_control::CoreAction::RecoverStart {
                    generation: Uuid::new_v4(),
                },
            },
            Operation::ControlCore {
                action: app_proxy_core::core_control::CoreAction::PrepareRemove {
                    expected_revision: 2,
    ...
    for operation in operations() {
        ...
        assert!(
            matches!(response.result, Reply::Error { code } if code == "CORE_PROTOCOL_UPDATE_REQUIRED")
        );
        assert!(
            fixture
                .shared
