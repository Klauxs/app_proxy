...
async function run(t:TestContext,steps:Step[],options:{profiles?:Profile[];discovered?:boolean;failVerify?:boolean;missingDesktop?:boolean}={}) {
  ...
  const s={
    apps:{desktop:async(kind:string)=>{calls.push('desktop:'+kind);if(options.missingDesktop)throw new Error('未找到桌面版');return {name:kind==='codex'?'Codex':'Claude',exe:`C:\\WindowsApps\\fixture\\${kind}.exe`,adapter:'chromium',args:[]};}},
    ...
    addApp:async(data:any)=>{calls.push('app');assert.ok(data.profileId===undefined||verified,'must verify before registering');apps.push(data);return {...data,id:'a',guard:data.adapter==='chromium'&&!!data.profileId};},
...
test('empty setup creates and verifies a manual proxy then binds the app without selecting a profile again',async t=>{
  const result=await run(t,[[/请选择/,'1'],[/选择应用：/,'3'],...manualSteps,...appSteps,[/请选择/,'0']]);
...
test('detected usable service goes straight to app setup',async t=>{
  const result=await run(t,[[/请选择/,'1'],[/选择应用：/,'3'],[/使用已有代理/,''],...appSteps,[/请选择/,'0']],{discovered:true});
...
test('a single saved proxy can be selected with Enter and is verified before adding',async t=>{
  const result=await run(t,[[/请选择/,'1'],[/选择应用：/,'3'],[/选择应用要使用的代理/,''],...appSteps,[/请选择/,'0']],{profiles:[profile]});
...
test('failed connectivity or cancelled setup never registers an app',async t=>{
  const failed=await run(t,[[/请选择/,'1'],[/选择应用：/,'3'],...manualSteps,[/请选择/,'0']],{failVerify:true});
  assert.equal(failed.apps.length,0);assert.match(failed.errors[0],/联网验证失败/);
  const cancelled=await run(t,[[/请选择/,'1'],[/选择应用：/,'3'],[/配置代理/,'0'],[/请选择/,'0']]);
...
test('direct mode requires an explicit choice and does not provision a proxy',async t=>{
  const result=await run(t,[[/请选择/,'1'],[/选择应用：/,'3'],[/配置代理/,'d'],...appSteps,[/请选择/,'0']]);
...
test('standalone proxy setup continues into adding an app with the new binding',async t=>{
  const result=await run(t,[[/请选择/,'3'],...manualSteps,[/代理已就绪，现在添加应用/,'y'],[/选择应用：/,'3'],...appSteps,[/请选择/,'0']]);
...
test('subscription setup downloads and selects nodes before verifying and continuing',async t=>{
  ...
  try {
    ...
    const result=await run(t,[[/请选择/,'1'],[/选择应用：/,'3'],[/配置代理/,''],[/代理名称/,''],[/本地监听端口/,''],[/订阅 URL/,`http://127.0.0.1:${address.port}/subscription`],[/国家\/地区编号/,'0'],[/节点编号/,'1'],...appSteps,[/请选择/,'0']]);
    assert.deepEqual(result.calls,['discover','create','verify','app']);assert.deepEqual(result.errors,[]);
  }finally{await new Promise<void>(resolve=>server.close(()=>resolve()));}
});
test('Codex/Claude presets only ask original or clone and a proxy, with no executable or adapter questions',async t=>{
  for(const [choice,kind] of [['1','codex'],['2','claude']])for(const mode of ['','1']) {
    const result=await run(t,[[/请选择/,'1'],[/选择应用：/,choice],[/使用方式/,mode],[/选择应用要使用的代理/,''],[/创建桌面快捷方式/,'n'],[/请选择/,'0']],{profiles:[profile]});
    assert.deepEqual(result.calls,['desktop:'+kind,'verify','app']);
    assert.equal(result.apps[0].instance,mode==='1'?kind:undefined);
    assert.equal(result.apps[0].adapter,'chromium');assert.deepEqual(result.apps[0].args,[]);
    assert.equal(result.apps[0].profileId,'p');assert.deepEqual(result.errors,[]);
  }
});
test('missing preset installation returns before creating a proxy or registering an app',async t=>{
  const result=await run(t,[[/请选择/,'1'],[/选择应用：/,'1'],[/请选择/,'0']],{missingDesktop:true});
  assert.deepEqual(result.calls,['desktop:codex']);assert.equal(result.apps.length,0);assert.match(result.errors[0],/未找到桌面版/);
});
