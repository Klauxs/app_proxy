...
test('elevated notification channel handles split messages and disconnects without sending commands',async()=>{
  ...
  const watcher=watchGuardEvents(native,'abcdef123456',event=>events.push(event),(state,reason)=>{ready=state;assert.equal(reason,'elevated-process-start');});
  try {
    await until(()=>!!client);
    client!.write('{"type":"rea');client!.write('dy","elevated":true}\n{"type":"start","pid":123,"name":"fixture.exe"}\n');
