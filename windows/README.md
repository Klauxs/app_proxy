...
1. 直接选择 **1「添加应用」**。工具先准备代理：有已保存配置时选择配置（只有一个可回车）；没有时自动发现并验证已有 sing-box HTTP/mixed 入口。
2. 没有可复用入口或选择配置新的代理时，就在当前流程填写订阅或手动 HTTP/SOCKS5 上游。订阅继续选择地区和节点；代理名称和空闲监听端口提供默认值，回车即可。需要 sing-box 程序时优先使用本机已有程序，其次随包副本，均缺失时协助安装。
3. 工具启动自建实例并验证选中的代理；自建实例按 Wi-Fi、其他物理网卡尝试，已有服务只验证，不改配置或生命周期。成功后直接进入应用信息填写，自动使用刚准备好的代理，不再询问“代理编号”。
4. 填写应用名称和实际 EXE，选择普通应用或分身及适配类型，按需创建桌面快捷方式。多个代理配置可以使用不同端口和出口。
...
添加时，确认使用 Chromium 适配的 `Codex.exe`、`Claude.exe`（不区分大小写）以及 Codex/Claude 分身，绑定代理后默认启用 Guard；菜单和 `app add` 行为一致，不再额外询问是否启用。按实际 EXE 判断，不使用可编辑的显示名；环境变量适配的同名 CLI 和直连应用不自动启用。`app.json` 可显式设置 `"guard":false` 跳过；之后可用 **6 Guard → 2 停用应用保护** 关闭。不批量修改已有登记。
