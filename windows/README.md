...
1. 进入“添加代理或订阅”，先发现并验证正在运行的 sing-box HTTP/mixed 入口，可直接选择复用；没有可用入口或选择跳过时，再导入订阅或填写手动 HTTP/SOCKS5 上游。
2. 自建配置需要分配本地监听端口，只监听 `127.0.0.1`；优先使用已有程序，找不到时使用随包程序，均缺失时安装官方版本。复用已有服务直接沿用其端口。
3. 自建实例启动时尝试 Wi-Fi、其他物理网卡，验证代理请求成功后采用；已有服务不改网卡、DNS 或任何配置。
4. “添加应用”中选择实际 EXE，选择适配类型和代理绑定，按需创建桌面快捷方式。多个代理配置可以使用不同端口和出口。
5. 对确认支持 `--proxy-server` 的 Chromium/Electron 应用，可启用 Guard。普通程序选择环境变量适配，不默认传入 Chromium 参数。
...
普通方式打开工具，在 **6 Guard → 1 启用应用保护** 中选择应用即可，首次启用会自动请求 Windows UAC 授权；不再单独选择监听权限。取消授权则不启用新的保护，也不修改原保护设置。只有通知辅助进程以管理员权限运行，Guard 和目标应用不因此提权；请使用当前 Windows 账号授权，该账号须属于管理员组。启用时会刷新普通权限 Guard 及登录任务，让新版监听接入。菜单“3 修复监听授权”仅用于修复或更新。
...
- 启动时扫描一次；优先监听 Windows 进程启动事件，收到已登记 EXE 的通知后立即检查同一用户、同一交互会话内的真实进程。事件模式每 30 秒扫描补漏。
- 监听统一使用管理员辅助进程，已删除普通权限监听路径。授权缺失、监听失败或中断时，后台临时退回约每 2 秒扫描，每 30 秒尝试恢复，不反复弹 UAC。`events.log` 中的 `guard-events-ready elevated-process-start` 表示已连接管理员监听，`guard-events-fallback` 标明降级原因。
...
当前事件源是 WMI `Win32_ProcessStartTrace`，不保证即时通知。8 个真实进程的分段测量显示：进程创建约 2–4 ms，创建至 WMI 生成通知约 1301–1996 ms，回调及管道接收约 0–3 ms，通知呈批量到达。该测试没有运行 Guard 扫描循环，因此约 2 秒并非扫描间隔或应用重启造成。提权解决访问权限，不消除 WMI 端的秒级延迟；本版未替换事件源。
