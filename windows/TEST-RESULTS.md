...
## Guard 刚启动直接结束、较老进程等待 500 ms（最新）

- 类型检查通过；Guard 事件与真实内核集成 12 项通过，原生停止策略测试 1 项通过，共 13 项（含子测试）。
- 原生停止策略直接执行生产停止分支，使用进程替身验证：5 秒内无代理主进程跳过窗口关闭；较老进程、代理参数存在、辅助进程或参数不可读时，Guard 路径最多正常等待 500 ms；非 Guard 路径保留 1500 ms。
- 真实集成确认 Guard 纠正正常工作，快速路径仍拒绝创建时间不符的进程身份。未用用户 Claude 进行强制重启测试；已同步当前安装并刷新 Guard。

## 按需等待与复用纠正检查（最新）

- 类型检查通过；Guard 事件、配置并发和真实内核集成测试共 14 项通过（含子测试），0 失败。
- 新增回归覆盖辅助进程立即退出、稍后退出、持续存活以及新主进程出现；仅清空后启动，持续存活时有限重试，新主进程不被结束或重复启动。复用检查仍拒绝不同应用对象及存活目标。
- 隔离真实集成日志确认启动阶段的重复包解析及进程检查耗时均为 0；一次纠正总耗时 1346 ms，其中清理扫描 311 ms，没有固定 300 ms 等待。这不是用户 Claude 的完整时延承诺。
- 已同步当前安装并刷新常驻 Guard，未主动触发用户 Claude 纠正。

## Guard 纠正等待与耗时日志（最新）

- 类型检查、10 项 Guard/MSIX 回归、2 项真实内核集成回归通过。集成包含应用启动、Guard 纠正、双出口、HTTPS、回滚及清理。
- 独立创建无窗口 Node 测试进程进行新旧关闭逻辑对照：2027 ms → 502 ms；省去无主窗口时无效的 1.5 秒等待。该数字不是 Claude 完整纠正耗时。
- 刷新现场常驻 Guard 后，日志确认 ETW 重新就绪；现有 Claude PID 49612 未变。未主动触发用户应用纠正，实际可见窗口体验及完整时延需以之后的 `guard-timing` 和实际启动为准。
